---
title: General
links:
  - href: https://mail.google.com/mail/u/0/#inbox
    title: Gmail
    srcLight: gmail-light.svg
    srcDark: gmail-dark.svg
    alt: gmail-icon
    category: mail
  - href: https://mail.proton.me/
    title: Proton Mail
    srcLight: proton-mail-light.svg
    srcDark: proton-mail-dark.svg
    alt: proton-mail-icon
    category: mail
  - href: https://app.fastmail.com/
    title: Fastmail
    srcLight: fastmail-light.svg
    srcDark: fastmail-dark.svg
    alt: fastmail-icon
    category: mail
  - href: https://www.messenger.com
    title: Messenger
    srcLight: facebook-messenger-light.svg
    srcDark: facebook-messenger-dark.svg
    alt: messenger-icon
    category: mail
  - href: https://voice.google.com
    title: Google Voice
    srcLight: google-voice-light.svg
    srcDark: google-voice-dark.svg
    alt: google-voice-icon
    category: mail
  - href: https://calendar.google.com
    title: Google Calendar
    srcLight: google-calendar-light.svg
    srcDark: google-calendar-dark.svg
    alt: google-calendar-icon
    category: google
  - href: https://drive.google.com/drive/u/0/my-drive
    title: Google Drive
    srcLight: google-drive-light.svg
    srcDark: google-drive-dark.svg
    alt: google-drive-icon
    category: google
  - href: https://photos.google.com/
    title: Google Photos
    srcLight: google-photos-light.svg
    srcDark: google-photos-dark.svg
    alt: google-photos-icon
    category: google
  - href: https://sheets.google.com
    title: Google Sheets
    srcLight: google-sheets-light.svg
    srcDark: google-sheets-dark.svg
    alt: google-sheets-icon
    category: google
  - href: https://my.1password.com/signin?l=en
    title: 1Password
    srcLight: 1password-light.svg
    srcDark: 1password-dark.svg
    alt: 1password-icon
    category: accounts
  - href: https://dash.cloudflare.com/login
    title: Cloudflare
    srcLight: cloudflare-light.svg
    srcDark: cloudflare-dark.svg
    alt: cloudflare-icon
    category: accounts
  - href: https://my.nextdns.io/3ec161/logs
    title: NextDNS
    srcLight: nextdns-light.svg
    srcDark: nextdns-dark.svg
    alt: nextdns-icon
    category: accounts
  - href: https://cloud.oracle.com/
    title: Oracle Cloud
    srcLight: oracle-light.svg
    srcDark: oracle-dark.svg
    alt: oracle-icon
    category: accounts
  - href: https://www.icloud.com/find
    title: Find My
    srcLight: icloud-light.svg
    srcDark: icloud-dark.svg
    alt: icloud-icon
    category: accounts
  - href: https://www.visible.com
    title: Visible
    srcLight: visible-by-verizon-light.svg
    srcDark: visible-by-verizon-dark.svg
    alt: visible-icon
    category: accounts
  - href: https://claude.ai/new
    title: Claude
    srcLight: claude-light.svg
    srcDark: claude-dark.svg
    alt: claude-icon
    category: dev
  - href: https://github.com/ryanchavezzz
    title: GitHub
    srcLight: github-light.svg
    srcDark: github-dark.svg
    alt: github-icon
    category: dev
  - href: https://news.ycombinator.com
    title: Hacker News
    srcLight: hacker-news-light.svg
    srcDark: hacker-news-dark.svg
    alt: hacker-news-icon
    category: dev
  - href: https://www.youtube.com/
    title: YouTube
    srcLight: youtube-light.svg
    srcDark: youtube-dark.svg
    alt: youtube-icon
    category: media
  - href: https://app.plex.tv/
    title: Plex
    srcLight: plex-light.svg
    srcDark: plex-dark.svg
    alt: plex-icon
    category: media
  - href: https://stash.rchvz.dev/
    title: Stash
    srcLight: stash-light.svg
    srcDark: stash-dark.svg
    alt: stash-icon
    category: media
  - href: https://sniffies.com
    title: Sniffies
    srcLight: sniffies-light.svg
    srcDark: sniffies-dark.svg
    alt: sniffies-icon
    category: media
  - href: https://www.printables.com
    title: Printables
    srcLight: printables-light.svg
    srcDark: printables-dark.svg
    alt: printables-icon
    category: printing
  - href: https://www.thingiverse.com
    title: Thingiverse
    srcLight: thingiverse-light.svg
    srcDark: thingiverse-dark.svg
    alt: thingiverse-icon
    category: printing
  - href: https://makerworld.com
    title: MakerWorld
    srcLight: maker-world-light.svg
    srcDark: maker-world-dark.svg
    alt: maker-world-icon
    category: printing
  - href: https://bambulab.com
    title: Bambu Lab
    alt: bambu-lab-icon
    category: printing
  - href: https://www.ebay.com
    title: eBay
    srcLight: ebay-light.svg
    srcDark: ebay-dark.svg
    alt: ebay-icon
    category: shopping
  - href: https://www.facebook.com/marketplace
    title: Facebook Marketplace
    srcLight: facebook-marketplace-light.svg
    srcDark: facebook-marketplace-dark.svg
    alt: facebook-marketplace-icon
    category: shopping
  - href: https://gay-torrents.net
    title: Gay-Torrents
    alt: gay-torrents-icon
    category: torrents
  - href: https://www.myanonamouse.net
    title: MyAnonamouse
    alt: myanonamouse-icon
    category: torrents
  - href: https://vinelink.vineapps.com/state/CA
    title: VINELink
    alt: vinelink-icon
    category: misc
  - href: https://brutalist.report/
    title: Brutalist Report
    alt: brutalist-report-icon
    category: misc
  - href: https://www.melissa.com/v2/lookups/
    title: Melissa Lookups
    alt: melissa-lookups-icon
    category: misc
---
